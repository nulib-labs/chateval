...
import pandas as pd
from chateval.chateval import get_token, confirm_login, get_answer, get_answers, format_answer, format_error, score_answer, score_answers_df, ask_claude
import json


@patch('chateval.chateval.webdriver.Chrome')
@patch('chateval.chateval.WebDriverWait')
...
def test_get_answer(mock_post):
    ...
    response = get_answer('test.com', 'test_question',
                          'fake_token', with_context=True)
    assert response.equals(pd.Series(['test_answer', 'test_context']))

    response = get_answer('test.com', 'test_question',
                          'fake_token', with_context=False)
...
@patch('chateval.chateval.get_answer')
def test_get_answers(mock_get_answer):
    ...
    result = get_answers("test.com", questions, token, with_context=False)
    assert result['answer'].iloc[0] == 'test_answer'


@patch('chateval.chateval.pkg_resources.open_text')
@patch('chateval.chateval.ask_claude')
def test_score_answer(mock_ask_claude, mock_open_text):
    ...
    question_answer_df = pd.Series(
        {'question': 'What is AI?', 'answer': 'Artificial Intelligence'})
    model = 'test_model'

    result = score_answer(question_answer_df, model)
    assert result.equals(
        pd.Series(['5', 'Good answer'], index=['score', 'reason']))


@patch('chateval.chateval.pkg_resources.open_text')
@patch('chateval.chateval.ask_claude')
@patch('chateval.chateval.tqdm.pandas')
...
@patch('chateval.chateval.boto3.client')
